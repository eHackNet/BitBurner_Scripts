...
export async function main(ns) {
    ...
    ns.print("ERROR  +---------------------------------+");
	ns.print("ERROR  |       Starting script here      |");
	ns.print("ERROR  +---------------------------------+");
    ns.print("");
    ns.print("");
    while (true) {
...
function tendStocks(ns) {
    ...
    ns.print('ERROR  +---------------------------------+');
    ns.print('ERROR  |         🌐 NET WORTH 🌐        |');
    ns.print('ERROR  +---------------------------------+');
    ns.print(`                  ${ns.nFormat(totalValue, "$0.000a")} `);
    ns.print(`           ${ns.nFormat(portfolioValue, "$0.0a")} stocks + ${ns.nFormat(cashValue, "$0.0a")} cash`);
    ns.print('ERROR  <--------------------------------->');
    ns.print('');
}
    ...
    for (const stock of stocksToSell) {
        if (stock.shares > 0) {
            ...
            if (salePrice != 0) {
                ...
                ns.print('INFO   |           ✅ SOLD ✅           |');
                ns.print('INFO   +---------------------------------+');
                ns.print(`            ${stock.summary} stock `);
                ns.print(`              for ${ns.nFormat(saleProfit, "$0.00a")} profit`);
                ns.print('INFO   <--------------------------------->');
                ns.print('');
            }
        }
...
function buyStocks(ns, stocksToBuy, maxTransactions=4) {
    ...
    for (const stock of bestStocks) {
        ...
        if (boughtPrice != 0) {
            ...
            ns.print('WARN   +---------------------------------+');
            ns.print('WARN   |          ⬇️ BOUGHT ⬇️          |');
            ns.print('WARN   +---------------------------------+');
            ns.print(`                   ${ns.nFormat(boughtTotal, "$0.000a")} of`);
            ns.print(`              ${stock.summary}`);
            ns.print('WARN  <--------------------------------->');
            ns.print('');
        }
    }
