...
export async function main(ns) {
    ...
    let pad = servers.reduce((acc, elem) => Math.max(acc, elem.length), 0)
		...
		ns.print("SUCCESS ║ G ║ " + latest[0][0] + latest[0][1].padStart(34 - latest[0][0].length) + " ║")

		ns.print("SUCCESS ║ W ║ " + latest[1][0] + latest[1][1].padStart(34 - latest[1][0].length) + " ║")

		ns.print("SUCCESS ║ H ║ " + latest[2][0] + latest[2][1].padStart(34 - latest[2][0].length) + " ║")


		ns.print("WARNING ║ " + cycle[cycle[0]] + " ╠════════════════════════════════════╣")
		...
		if (targetList.length < 6) { ns.print("╚═══╝                                    ║") } else {
			...
			for (let i = 0; i < 6; i++) {
				...
				ns.print("INFO    ║ > " + temp + logBalance(temp).padStart(36 - temp.length) + " ║")

			}

			//ns.print("ERROR   ╠════════════════════════════════════════╣")

			ns.print("SUCCESS ╠═══╛ EXE " + exes.length + "/5 ║ HOSTS " + hostList.length + " ║ TARGETS " + targetList.length + " ╘══╣")

			//ns.print("ERROR   ╠════════════════════════════════════════╣")

			if (manager) {

				ns.print("WARNING ╚══════╕ Managing " + ns.hacknet.numNodes() + " HNet Nodes ╒".padEnd(21, "═") + "╝")
