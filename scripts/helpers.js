...
export async function getNsDataThroughFile_Custom(ns, fnRun, fnIsAlive, command, fName, verbose = false, maxRetries = 5, retryDelayMs = 50) {
    ...
    fName = fName || `/scripts/Temp/${commandHash}-data.txt`;
    const fNameCommand = (fName || `/scripts/Temp/${commandHash}-command`) + '.js'
...
export async function runCommand_Custom(ns, fnRun, command, fileName, verbose = false, maxRetries = 5, retryDelayMs = 50, ...args) {
    ...
    let script = `import { formatMoney, formatNumberShort, formatDuration, parseShortNumber, scanAllServers } fr` + `om '${getFilePath('/scripts/helpers.js')}'\n` +
    ...
    fileName = fileName || `/scripts/Temp/${hashCode(command)}-command.js`;
...
export async function getActiveSourceFiles_Custom(ns, fnGetNsDataThroughFile) {
    ...
    let tempFile = '/scripts/Temp/owned-source-files.txt';
    ...
    dictSourceFiles[(await fnGetNsDataThroughFile(ns, 'ns.getPlayer()', '/scripts/Temp/player-info.txt')).bitNodeN] = 3;
...
export async function tryGetBitNodeMultipliers_Custom(ns, fnGetNsDataThroughFile) {
    ...
    try { return await fnGetNsDataThroughFile(ns, 'ns.getBitNodeMultipliers()', '/scripts/Temp/bitnode-multipliers.txt'); } catch { }
    return null;
}
